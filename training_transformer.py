...
class TrainTransformer:
    ...
    def train(self, args):
        train_dataset = load_video_data(args)
        steps_per_epoch = len(train_dataset)
        current_time = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
        writer = SummaryWriter(f'tensorboard_logs/ffhq_transformer/{current_time}')
        for epoch in range(args.epochs):
            with tqdm(range(len(train_dataset))) as pbar:
                for i, imgs in zip(pbar, train_dataset):
                    self.optim.zero_grad()
                    imgs = imgs.to(device=args.device)
                    logits, targets, reconstruct = self.model(imgs)
                    ...
                    if i % 20 ==0:
                        with torch.no_grad():
                            real_fake_images = torch.cat((imgs[:4,-1,:].squeeze(), reconstruct[:4]))
                            vutils.save_image(real_fake_images, os.path.join("results_transformer", f"{epoch}_{i}.jpg"), nrow=4)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset-path', type=str, default='/home/xuyang/VQGAN-pytorch/data/1ki58847CNs/frames/00015/', help='Path to data.')
    parser.add_argument('--checkpoint-path', type=str, default='/home/xuyang/VQGAN-pytorch/checkpoints/vqgan_epoch_99.pt', help='Path to checkpoint.')
    parser.add_argument('--device', type=str, default="cuda:0", help='Which device the training is on')
    parser.add_argument('--batch-size', type=int, default=8, help='Input batch size for training.')
    parser.add_argument('--num-frames', type=int, default=6, help='Number of frames in the video.')
