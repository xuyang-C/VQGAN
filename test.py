...
def test(args):
    ...
    psnr_list = []
    msssim_list = []
    os.makedirs("data/1ki58847CNs/frames/reconstruct-after5s/after5s", exist_ok=True)
    with tqdm(range(len(test_dataset))) as pbar: # 由于需要保存图片，batchsize=1
        ...
        # for i in range(len(test_dataset)):
            ...
            # save_image(real_fake_images, modified_path ,nrow=args.batch_size)
            ...
            mse = torch.mean((imgs - decoded_images) ** 2,dim=[1,2,3]) # [batch_size] 不对batch维度求均值
            psnr = 10 * torch.log10(1 / mse)
            avg_psnr=torch.mean(psnr).item()
            # mse_ = torch.nn.functional.mse_loss(imgs, decoded_images)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image-size', type=int, default=240, help='Image height and width (default: 256)')
    ...
    parser.add_argument('--dataset-path', type=str, default='/home/xuyang/VQGAN-pytorch/data/UCF50/HorseRiding/test_', help='Path to data (default: /data)')
    ...
    parser.add_argument('--reload', type=str, default='/home/xuyang/VQGAN-pytorch/checkpoints-8192-Horse-128dim/vqgan_epoch_85.pt', help='Reload model from checkpoint')
